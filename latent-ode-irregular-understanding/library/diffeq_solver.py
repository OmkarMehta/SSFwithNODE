...
class DiffeqSolver(nn.Module):
	...
	def forward(self, first_point, time_steps_to_predict, backwards = False):
		...
		print(f"first_point.shape is {first_point.shape}")

		pred_y = odeint(self.ode_func, first_point, time_steps_to_predict, 
			rtol=self.odeint_rtol, atol=self.odeint_atol, method = self.ode_method)
		print(f"pred_y.shape from odeint is {pred_y.shape}")
		pred_y = pred_y.permute(1,2,0,3)
		print(f"pred_y.shape after permute is {pred_y.shape}")
		print(f"Should be shape: [n_traj_samples, n_traj, n_tp, n_dim]")
		print(f"\n")
