...
# Author: Omkar Mehta
# Adapted from code by: Latent ODEs for Irregularly-Sampled Time Series
###########################
...
def split_data_extrap(data_dict, dataset = ""):
    ...
    n_observed_tp = data_dict["data"].size(1) // 2 # lag
    if dataset == "hopper":
        n_observed_tp = data_dict["data"].size(1) // 3

    split_dict = {"observed_data": data_dict["data"][:,:n_observed_tp,:].clone(), # (n_batches, lag, input_dim)
                "observed_tp": data_dict["time_steps"][:n_observed_tp].clone(), # lag
                "data_to_predict": data_dict["data"][:,n_observed_tp:,:].clone(), # (n_batches, forecast, input_dim)
                "tp_to_predict": data_dict["time_steps"][n_observed_tp:].clone() # forecast
                }
...
def split_data_interp(data_dict):
    ...
    split_dict = {"observed_data": data_dict["data"].clone(), # (n_batches, lag, input_dim)
                "observed_tp": data_dict["time_steps"].clone(), # lag
                "data_to_predict": data_dict["data"].clone(), # (n_batches, lag, input_dim)
                "tp_to_predict": data_dict["time_steps"].clone()} # lag
...
def create_net(n_inputs, n_outputs, n_layers = 1, 
	...
	layers = [nn.Linear(n_inputs, n_units)] # (input_dim, n_units)
	for i in range(n_layers):
		layers.append(nonlinear())
		layers.append(nn.Linear(n_units, n_units))

	layers.append(nonlinear())
	layers.append(nn.Linear(n_units, n_outputs)) # (n_units, output_dim)
